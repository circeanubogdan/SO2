...
int main(int argc, char *argv[]) {
	...
	if (mode == 0) {
		src_p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, src_fd, 0);
		if (src_p < 0)
			return -1;

	}

	dst_fd = open(argv[4], O_CREAT | O_RDWR | O_TRUNC, 0644);
	if (dst_fd < 0)
		return -1;
	...
	if (mode == 0) {
		dst_p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, dst_fd, 0);
		if (dst_p < 0)
			return -1;
	}
	...
	if (mode == 0) {
		...
		pread(src_fd, buf, to_write, size - to_write);
		pwrite(dst_fd, buf, blk_size, size - to_write);
	}
